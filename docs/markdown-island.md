...
### The interactive highlight bridge

A rendered table and a collapsed image source label both replace real buffer text with their own DOM (`Decoration.replace`, see the table section above); a `Decoration.mark` Neovim highlight computes for that text (Search, IncSearch, an EasyMotion target, hop.nvim's `HopPreview` pattern-typing highlight) has nothing left to attach to once the text it covers is gone. `island-display-decorations.js` filters `hl.runs` for exactly those groups into one `interactiveHighlights` list per push and dispatches it as a single `setInteractiveHighlights` effect that both `markdownTableField` and `markdownImageField` (`markdown-presentation.js`) read independently, each mapping the buffer-relative runs onto its own cell or caption offsets (`tableHighlightCells` / `imageCaptionHighlights`, `pure/markdown.js`) and rendering them inline via a shared `appendHighlightedText` helper. `EasyMotionShade` and hop's own `HopUnmatched` dimming are deliberately excluded, the same way `EasyMotionShade` already was, since either would dim every cell or the whole caption. hop's *second* step, the per-target hint letters, is a `virt_text` overlay (`hl.virt`, a different payload and a different `OverlayWidget` entirely) and is not part of this bridge; it still does not render inside a hidden table or image caption, since an overlay decoration on hidden text has the same "nothing left to attach to" problem this bridge exists to solve, just not solved for that case yet.
...
- `src/markdown-presentation.js` / `src/pure/markdown.js`: `markdownTableField` / `markdownImageField`, `MarkdownTableWidget` / `MarkdownImageSourceWidget`, the shared `appendHighlightedText` interactive highlight bridge, `tableHighlightCells` / `imageCaptionHighlights`.
