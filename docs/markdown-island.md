...
- folds: `foldclosed` / `foldclosedend` and `foldlevel` inside `nvim_win_call`, walking the padded viewport and emitting `{ startRow, endRow, closed }` for each fold region. Source agnostic: whatever `foldmethod` and `foldtext` the user set is what shows.
...
**Built-in search.** Neovim does not expose `'hlsearch'` or `'incsearch'` matches through `getmatches()`: they are separate internal display layers. For settled searches, when `v:hlsearch` says highlighting is currently active, the island reads the `/` register and resolves it over the padded viewport with `matchbufline()`, preserving Vim regexp semantics, then emits those spans in the `Search` highlight group.

While Neovim owns an active `/` or `?` command line and `'incsearch'` is on, the island instead reads `getcmdline()` on every `CmdlineChanged`. It removes only an unescaped trailing search delimiter and offset, then gives the remaining expression to Neovim's own `matchbufline()` rather than parsing or evaluating the Vim regexp in JavaScript. Neovim temporarily places its window cursor just after the active incremental match; that position identifies the one span receiving `IncSearch`, including backward search, wrapping, `CTRL-G`, and `CTRL-T`. If `'hlsearch'` is also active, every incremental match receives `Search`; otherwise only the current `IncSearch` span appears. The same payload carries that temporary position separately from the real editing cursor. The island applies its pixel scrolloff visibility calculation to this position without changing its CodeMirror selection or cursor decoration, so distant and cycled incremental results become visible. `CmdlineLeave` restores the settled search state and follows the authoritative real cursor after Enter or Escape.

Rendered HTML tables own replacement text nodes, which CodeMirror marks around the hidden Markdown source cannot reach. Their existing interactive-highlight bridge therefore also receives `Search` and `IncSearch` runs. Overlapping groups share one table text span and CSS priority decides the visible style, so `IncSearch` wins on the current result while settled search and EasyMotion targets remain deterministic.

Each table cell is also parsed as GFM inline Markdown when the replacement DOM is built. Strong and emphasis marks, strikethrough, inline code, links, autolinks, and resolvable inline images become their corresponding DOM elements; the source delimiters are omitted from the visible cell. Cell content is still limited to the existing one-line table grammar, so lists, blockquotes, and code blocks are intentionally not introduced as a multiline table extension. The renderer constructs nodes and assigns text through DOM properties rather than injecting `innerHTML`.

The cache key includes settled and incremental patterns plus the active incremental result position, so typing another character or selecting another result changes the island even when neither the buffer nor plugin matches changed. Search is layered above syntax and extmarks but below explicit `:match` overlays, keeping interactive plugin hints visible.
...
`collect_folds` walks the padded viewport with `foldclosed` / `foldclosedend` for closed folds and `foldlevel` for open folds. It emits `{ startRow, endRow, closed }`, using the fold's first line as the start according to the same level transition that defines the region. The real fold bounds are used even when they extend past the padded range. The client uses every first line for the gutter underline, while only closed regions hide their bodies and receive the closed text treatment.
...
Display only. Open a fold from Neovim (`zo`), and the next push reveals its body while retaining the first-line gutter underline.
...
A whole logical line containing `![alt](url)` gets a noneditable block `figure` widget immediately after the source line. Away from the cursor, its source line is replaced with a friendly label. When the cursor enters its line, the raw Markdown returns for direct editing, preserving the normal Neovim cursor, undo, grammar integration, and exact source position. Inline `![alt](url)` in ordinary prose remains text, while the table renderer handles resolvable inline images inside table cells. `[text](url)` remains a link even if its destination is an image.

The widget contains a semantic `img`; it has no `figcaption` because the visible alt text on the source line is already the image label. `![label|600](url)` adds one optional width in CSS pixels: the image gets a `width: 600px` style and retains its intrinsic ratio, but the source label renders as the literal `label|600`, unmodified. An earlier version rendered it as the friendlier `label (600px)`, but that fabricates characters (`(`, `px`, `)`) that do not exist in the buffer, which broke Neovim search and hop.nvim matches run against that text (see the highlight bridge below); the label is deliberately not the same string the width styling reads. Remote `http` and `https` sources are passed directly to the webview. Local, relative sources are resolved against the current buffer name and passed through Tauri's asset protocol with `convertFileSrc`, not against the webview URL. The snapshot's existing `name` field is therefore also the image base path. `tauri.conf.json` enables that protocol for absolute paths because a Markdown file can be opened from anywhere, including `/private/tmp`; its CSP explicitly permits `asset:`, `http:`, and `https:` image sources.
...
### The interactive highlight and overlay bridge

A rendered table and a collapsed image source label both replace real buffer text with their own DOM (`Decoration.replace`, see the table section above); a Neovim decoration computed for that text has nothing left to attach to once the text it covers is gone, whether it colours the text (`Decoration.mark`: Search, IncSearch, an EasyMotion target, hop.nvim's `HopPreview` pattern-typing highlight) or replaces it outright (`Decoration.replace` widget: hop's own per-target hint letters, a `virt_text` overlay). `island-display-decorations.js` builds two lists per push, filtering `hl.runs` for exactly those highlight groups into `interactiveHighlights` and passing `hl.virt` through as `interactiveOverlays` (every entry is a candidate; row/cell matching happens downstream), and dispatches each as its own effect (`setInteractiveHighlights`, `setInteractiveOverlays`) that both `markdownTableField` and `markdownImageField` (`markdown-presentation.js`) read independently. Each field maps the buffer-relative runs onto its own cell or caption offsets (`tableHighlightCells` / `imageCaptionHighlights` for colour, `tableOverlayCells` / `imageCaptionOverlays` for hint letters, all in `pure/markdown.js`) and renders them inline via a shared `appendHighlightedText` helper, where an overlay's replacement text always wins over a highlight colour on the same characters, matching the non-table/image `OverlayWidget` elsewhere in this codebase. `EasyMotionShade` and hop's own `HopUnmatched` dimming are deliberately excluded from the highlight list, since either would dim every cell or the whole caption.
A table gets one additional narrow highlight feed for inline code and math only: code runs are selected by overlap with `hl.codespans`, while math runs are selected by their math highlight group. Other ordinary syntax runs remain excluded from replacement table content, preserving the live preview's intentionally lighter styling policy.
...
- `src/markdown-presentation.js` / `src/pure/markdown.js`: `markdownTableField` / `markdownImageField`, `MarkdownTableWidget` / `MarkdownImageSourceWidget`, the shared `appendHighlightedText` interactive highlight/overlay bridge, `tableHighlightCells` / `imageCaptionHighlights` / `tableOverlayCells` / `imageCaptionOverlays`.
