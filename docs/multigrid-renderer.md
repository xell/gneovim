...
`gnv_cursor` (the buffer position) and `grid_cursor_goto` (which grid owns it) are two independent streams, and nothing guarantees their arrival order. A plugin that blocks on `getchar()` to show a prompt (vim-easymotion's target-letter overlay is the case that surfaced this) can make Neovim send a `grid_cursor_goto` to a different grid for the duration, clearing the island's cursor decoration when `cursorGrid` moves away from it. When the prompt resolves and the cursor lands at its final position in the island's window, if the `gnv_cursor` event for that move is processed *before* the `grid_cursor_goto` that hands the grid back, the island's cursor listener finds `islandForGrid(cursorGrid)` still pointing elsewhere, silently drops the update, and nothing else re-fires it: the cursor stays invisible until an unrelated move (`h` / `l`) re-triggers both streams in a working order.

Fixed by caching the latest `gnv_cursor` payload (`lastCursorPayload`) regardless of whether an island currently owns `cursorGrid`, and replaying it the moment `grid_cursor_goto` hands an island its grid back, instead of waiting for a fresh `gnv_cursor` event that Neovim may not consider necessary to resend.
...
- **A DOM update can sit applied but unpainted until something forces a reflow.** Same family of bug as the point above, hit again in the markdown island: a CodeMirror decoration update (hop.nvim's jump-letter overlay, landing on the debounced `md_decor` push a beat after `<CR>` confirms the search) was applied to the document but never composited, invisible until opening devtools (itself a resize) forced a repaint. Grid windows already work around this with `forceRepaint` (toggle `display:none` then restore, forcing a reflow) after every dirty-row repaint; `Island.applyDecor` now does the same after dispatching a decoration change. The toggle would blur a focused `.cm-content` (an island descendant), since `display:none` removes it from the render tree, so it snapshots `hasFocus` first and re-focuses after if it was set; cheap and safe even mid insert-mode typing.
- **`overflow: hidden` does not stop WebKit's own focus/selection auto-scroll.** `#viewport` holds every grid and island, all `position: absolute`, and was never meant to scroll at all: real scrolling happens inside an island's own `.cm-scroller`. It was `overflow: hidden`, which hides the scrollbar and blocks user-driven scroll gestures but still leaves the element a scroll container WebKit can scroll programmatically, including its own "bring the focused or selected node into view" behaviour. CodeMirror moving the cursor to a DOM node that was previously far outside any rendered viewport (`gg` in a long, scrolled island) triggered exactly that: WKWebView set a real `scrollTop` on `#viewport`, shifting every absolutely positioned child up by that amount, which clipped grid 1's row 0, the tabline, out of view, confirmed live (`#viewport.scrollTop` read a nonzero value at the exact moment the tabline visually disappeared). A first fix reacted to it with a `scroll` listener that reset `scrollTop` back to `0`; that fought CodeMirror's own `getBoundingClientRect`-based scroll math against the same ancestor on every cursor move, visible as the whole window shaking on `j` / `k`. Fixed at the actual root instead: `overflow: clip` removes scroll-container semantics entirely, so there is nothing left for either WebKit's auto-scroll or a reactive listener to fight over.
