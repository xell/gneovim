...
class Island {
  ...
  verticalMotionTarget(forward, count = 1) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // A single moveVertically step should only ever land on the same
      // source line (moving within a wrapped paragraph) or the line
      // immediately adjacent to it -- every real display-line step in this
      // buffer is exactly one of those two. A step that lands more than one
      // source line away means moveVertically silently jumped over real
      // content instead of landing on it. Confirmed live two different ways:
      // over a rendered table/image block widget (posAtCoords's scanY
      // handling in @codemirror/view's cursor.ts treats a non-text block as
      // a candidate to scan past, not land on), and separately -- 2026-09-21,
      // Leo's test1.md lines 125-142 -- over six lines of plain, fully
      // visible list text with no widget or fold involved at all: gk from
      // just below the list, once the target line had scrolled to sit right
      // at the edge of the measured viewport, returned a position 8-9 source
      // lines away in one call (goalColumn was fine; only the landing line
      // was wrong). Rather than special-case every way moveVertically can
      // misfire, check the one invariant that holds across all of them and
      // bail to the caller's native-forward fallback below, which runs
      // Neovim's own line-by-line motion -- unaffected by any of this
      // client-side rendering -- one row at a time, same as before this
      // feature existed.
      const fromLine = doc.lineAt(pos);
      const toLine = doc.lineAt(next.head);
      if (Math.abs(toLine.number - fromLine.number) > 1) break;
      pos = next.head;
...
(async function boot() {
  ...
  await Promise.all([
    ...
    // A stale island: either `:GneovimResyncIsland`'s fold-desync escape
    // hatch (win 0, every island) or an automatic reload nvim_buf_attach
    // cannot see -- `:checktime` / autoread / `:edit!` (a specific win, just
    // that island). See docs/markdown-island-fold-desync.md and
    // runtime/md_preview.lua / runtime/md_decor.lua. Either way the Lua side
    // has already reset its own fold and highlight-cache state before
    // sending this; the detach/reattach here is the same fresh-snapshot path
    // applyBufLines already falls back to on a caught desync.
    nvim.on("resync_island", (e) => {
      const win = e.payload;
      if (win) {
        jlog(`resync_island: refreshing window ${win}`);
        islandManager.resyncWindow(win);
      } else {
